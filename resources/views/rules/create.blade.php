<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-slate-800 leading-tight">
                <i class="fas fa-plus-circle text-green-600 mr-2"></i> {{ __('Add New Role') }}
            </h2>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-slate-200 uppercase">
                <div class="p-6 bg-white border-b border-slate-200">
                    <div class="flex items-center space-x-2 mb-6">
                        <i class="fas fa-id-badge text-slate-400"></i>
                        <h3 class="text-lg font-medium text-slate-800">{{ __('New Role Details') }}</h3>
                    </div>
                    ...
                    <form action="{{ route('rule.store') }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="title" name="title" type="text" class="mt-1 block w-full"
                                placeholder="Enter role title" required autofocus />
                        ...
                        <div class="flex items-center justify-center pt-4">
                            <button
                                class="inline-flex items-center px-6 py-3 bg-green-600 border border-transparent rounded-xl font-semibold text-sm text-white uppercase tracking-widest hover:bg-green-700 active:bg-green-800 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition ease-in-out duration-150 shadow-lg shadow-green-600/20">
                                <i class="fas fa-save mr-2"></i> {{ __('Create New Role') }}
                            </button>
                        </div>
                    </form>
                </div>
